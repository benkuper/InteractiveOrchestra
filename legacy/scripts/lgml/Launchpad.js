...
function updatePadColor(row, column)
{
	if(isTrack(row, column))
	{
		...
		setPadColor(row, column, getColorForTrackState(looperIndex, getTrackOnPad(row, column).state.get()));
	}else
	{
		if(row == 0 && column == 3)
		{
		...
		{
			setPadColor(row, column, recorderNode.getTarget().isRecording.get()?[1,0,0]:[0,0,0]);
		}else if(row == 0 && column < NUM_MICS)
		{
			setPadColor(row, column, audioIn.getTarget().channels.getChild(column+1).active.get()?[1,.5,0]:[0,0,0]);
		}
	}
...
function isOneTrackPlaying(looperIndex)
{
	...
	{
		var s = getTrack(looperIndex,i).state.get();
